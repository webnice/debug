...
// Empty function
// if set environment value DEBUG_CALLS=1 all calls Nop() displayed filename and line
func Nop() {
	if seeCalls {
		var self *debug = newDebug()
		self.ReadWriter.WriteString(fmt.Sprintf("[ %30s ] %s:%d", "debug.Nop()", self.Trace.File, self.Trace.Line))
...
// Name file is the same program name and .txt at extension
func DumperFile(idl ...interface{}) {
	FileSave(newDebug().Prefix("debug.DumperFile()").Dump(idl...).Suffix().Final().Bytes(), "", "")
}

// Enable or disable call stack trace
func SeeCallStack(v bool) {
	seeTrace = v
	if seeCalls {
		var self *debug = newDebug()
		self.ReadWriter.WriteString(fmt.Sprintf("[ %30s ] %s:%d", "debug.SeeCallStack()", self.Trace.File, self.Trace.Line))
		fmt.Print(self.Final().String())
	}
}
